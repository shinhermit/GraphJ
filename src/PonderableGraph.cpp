...
void PonderableGraph::remove_node(Node::node_id id){
  std::map<Edge, Edge::Cost>::iterator it;
...
void PonderableGraph::add_edge(Node::node_id id1, Node::node_id id2, PonderableGraph::Cost cost){
  std::map<Node::node_id, std::set<WeightedNode> >::iterator it;

  if(cost < 0) throw std::invalid_argument("PonderableGraph::add_edge(node_id, node_id, int): argument 3 of type Edge::Cost may not be negative");

  DirectableGraph::add_edge(id1, id2);
  if(_edgeState == EdgeState::WEIGHTED){
...
void PonderableGraph::remove_edge(Node::node_id id1, Node::node_id id2){
  ...
  if(_edgeState == EdgeState::WEIGHTED){
    ...
    if( it == _weights.end() && _edgeType == DirectableGraph::EdgeType::UNDIRECTED){
...
void PonderableGraph::setCost(Node::node_id node1, Node::node_id node2, PonderableGraph::Cost cost){
  std::map<Edge, Cost>::iterator it;

  if( !has_edge(node1, node2) ){
    throw std::invalid_argument("PonderableGraph::setCost(node_id, node_id, PonderableGraph::Cost): given node_ids do not refer to a valid edge");
  }
  else if(_edgeState == EdgeState::UNWEIGHTED){
    throw std::logic_error("PonderableGraph::setCost(node_id, node_id, PonderableGraph::Cost): the graph is an unweighted Graph");
  }
  else if(cost < 0){
    throw std::invalid_argument("PonderableGraph::setCost(node_id, node_id, int): argument 3 of type PonderableGraph::Cost may not be negative");
  }
  else{
...
PonderableGraph::Cost PonderableGraph::getCost(Node::node_id node1, Node::node_id node2)const{
  std::map<Edge, Cost>::iterator it;
