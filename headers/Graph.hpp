...
public:
  Graph(const GraphTypes::EdgeType & edgeType=GraphTypes::UNDIRECTED,
  ...
  void add_edge(const GraphTypes::node_id & src_node,
		const Type & content1,
		const GraphTypes::node_id & target_node,
		const Type & content2,
		const GraphTypes::Cost & cost=0.F);

  void add_edge(const GraphTypes::node_id & existing_src,
  ...
  void set_node_content(const GraphTypes::node_id & node,
			const Type & content
